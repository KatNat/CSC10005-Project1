...
void QInt::_string_div_two(std::string& value) {
	std::string tmp = "";
...
int QInt::_multiply(int x, int res[], int size) {
	int carry = 0;
...
std::string QInt::_power(int x, int n) {
...
std::string QInt::_add_string(std::string a, std::string b) {
...
std::string QInt::bin_to_dec(std::string value) {
	...
	for (int i = value.length() - 1; i >= 0; i--) {
		if (value[i] == '1')
			res = _add_string(res, _power(2, count));
		count++;
	}
	return res;
}
...
std::string QInt::dec_to_hex(std::string value) {
	return std::bitset<_BINARY_LENGTH>(bin_to_hex(dec_to_bin(value))).to_string();
}
